 ...
 * @returns number
 */
export const widthComposer = function widthComposer(width) {
 ...
 * @returns number
 */
export const heightComposer = function heightComposer(height) {
 ...
 * @returns number
 */
export const pxToDeviceRatio = function pxToDeviceRatio(val, side) {
...
export const fetchComponent = function(fileName) {
    const script = document.createElement('script');
    script.src = `./src/compFolder/${fileName}.js`;
    script.type = 'module'

    script.onerror = function() {
        console.info(`%c fetchComponent() : Failed`,'color:red')
    }

    script.onload = function() {
        console.info(`%c ${fileName} Component Loaded`,'color:green')
    }
 ...
 */
export const loadScript = function(filePath, callback) {
    const script = document.createElement('script');
    script.src = filePath;
    script.onerror = function() {
        console.info(`%c ui.loadScript() : Failed`,'color:red')
    }
    if(callback) {
        script.onload = function() {
            console.info(`%c ui.loadScript() : ${filePath}`,'color:green')
            callback()
        }
    } else {
        console.info(`%c ui.loadScript() : ${filePath}`,'color:green')
 ...
 */
export const loadCSS = function(filepath) {
    ...
    fileref.onerror = function(){
        console.info('%c loadCSS() : Failed','color:red')
    }
    fileref.onload = function(){
        console.info('%c loadCSS() : Loaded','color:green')
    }
...
export const ui = new function() {
    ...
    this.addLayout = (layout) => {
        if(layout.element instanceof HTMLElement) {
            ...
            console.info(`%caddLayout() : ${layout.element}`,'color:purple')
        } else {
            console.info(`%cThe Returned Object Is, \nNot An HTMLElement : ${layout.element}`,'color:red')
        }
    }
}
/**
 ...
 * use the available js dom methods to that element.
...
const htmlElement = class extends ElementComposer {
    constructor(parent, element, width, height, options) {
        ...
        this.logName = function(){
            let firstLetter = this.HTMLElement.at(0).toUpperCase();
            let otherLetters = this.HTMLElement.substr(1,this.HTMLElement.length);
 ...
 */
export const createSignal = function(defaultVal = null) {
 ...
 * @returns object
 */
export const createState = function(initialState) {
...
document.body.onload = function() {
    console.info(`%cinnerscope running on :${platform.type()}`,'color:orange');
    const App = new Application;
    App.onStart()
}
